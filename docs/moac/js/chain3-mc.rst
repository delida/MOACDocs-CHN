...
此程序包的功能返回的所有帐号地址均是带有校验和的地址返回。
这意味着有些字母是大写字母，有些字母是小写字母。
不正确的校验和地址在传递给函数时将引发错误。
如果要绕过校验和检查，可以将地址全部大写或小写。
...
For ``chain3.mc.subscribe`` see the :ref:`Subscribe reference documentation <mc-subscribe>`
...
For ``chain3.mc.Contract`` see the :ref:`Contract reference documentation <mc-contract>`
...
For ``chain3.mc.Iban`` see the :ref:`Iban reference documentation <mc-iban>`
...
For ``chain3.mc.personal`` see the :ref:`personal reference documentation <mc-personal>`
...
For ``chain3.mc.accounts`` see the :ref:`accounts reference documentation <mc-accounts>`
...
For ``chain3.mc.ens`` see the :ref:`Ens reference documentation <mc-ens>`
...
For ``chain3.mc.abi`` see the :ref:`ABI reference documentation <mc-abi>`
...
For ``chain3.mc.net`` see the :ref:`net reference documentation <mc-net>`
...
.. code-block:: javascript
    ...
    chain3.mc.getHashrate().then(console.log);
    > 493736
...
.. code-block:: javascript
    ...
    chain3.mc.getGasPrice().then(console.log);
    > "20000000000"
...
This means you can add accounts with :ref:`chain3.mc.accounts.create() <accounts-create>` and you will get them returned here.
...
.. code-block:: javascript
    ...
    chain3.mc.getBlockNumber().then(console.log);
    > 2744
...
2. ``Number|String`` - (optional) If you pass this parameter it will not use the default block set with :ref:`chain3.mc.defaultBlock <mc-defaultblock>`.
...
3. ``Number|String`` - (optional) If you pass this parameter it will not use the default block set with :ref:`chain3.mc.defaultBlock <mc-defaultblock>`.
...
1. ``String|Number`` - The block number or block hash. Or the string ``"genesis"``, ``"latest"`` or ``"pending"`` as in the :ref:`default block parameter <mc-defaultblock>`.
...
1. ``String|Number`` - The block number or hash. Or the string ``"genesis"``, ``"latest"`` or ``"pending"`` as in the :ref:`default block parameter <mc-defaultblock>`.
...
``Promise<object>`` - The returned uncle. For a return value see :ref:`chain3.mc.getBlock() <mc-getblock>`.
...
1. ``String`` - A block number or hash. Or the string ``"genesis"``, ``"latest"`` or ``"pending"`` as in the :ref:`default block parameter <mc-defaultblock>`.
...
``Promise<object>`` - A transaction object, see :ref:`chain3.mc.getTransaction <mc-gettransaction-return>`:
...
1. ``Object`` - The transaction object to send:
  - ``from`` - ``String|Number``: The address for the sending account. Uses the :ref:`chain3.mc.defaultAccount <mc-defaultaccount>` property, if not specified. Or an address or index of a local wallet in :ref:`chain3.mc.accounts.wallet <eth_accounts_wallet>`.
  ...
  - ``gasPrice`` - ``Number|String|BN|BigNumber``: (optional) The price of gas for this transaction in :ref:`wei <what-is-wei>`, defaults to :ref:`chain3.mc.gasPrice <mc-gasprice>`.
...
.. note:: The ``from`` property can also be an address or index from the :ref:`chain3.mc.accounts.wallet <eth_accounts_wallet>`. It will then sign locally using the private key of that account, and send the transaction via :ref:`chain3.mc.sendSignedTransaction() <mc-sendsignedtransaction>`.
...
``PromiEvent``: A :ref:`promise combined event emitter <promiEvent>`. Will be resolved when the transaction :ref:`receipt <mc-gettransactionreceipt-return>` is available. Additionally the following events are available:

- ``"transactionHash"`` returns ``String``: Is fired right after the transaction is sent and a transaction hash is available.
- ``"receipt"`` returns ``Object``: Is fired when the transaction receipt is available.
- ``"confirmation"`` returns ``Number``, ``Object``: Is fired for every confirmation up to the 12th confirmation. Receives the confirmation number as the first and the :ref:`receipt <mc-gettransactionreceipt-return>` as the second argument. Fired from confirmation 0 on, which is the block where its minded.
...
Sends an already signed transaction, generated for example using :ref:`chain3.mc.accounts.signTransaction <mc-accounts-signtransaction>`.
...
``PromiEvent``: A :ref:`promise combined event emitter <promiEvent>`. Will be resolved when the transaction :ref:`receipt <mc-gettransactionreceipt-return>` is available.

Please see the return values for :ref:`chain3.mc.sendTransaction <mc-sendtransaction-return>` for details.
...
1. ``String`` - Data to sign. If String it will be converted using :ref:`chain3.utils.utf8ToHex <utils-utf8tohex>`.
2. ``String|Number`` - Address to sign data with. Or an address or index of a local wallet in :ref:`chain3.mc.accounts.wallet <eth_accounts_wallet>`.
3. ``Function`` - (optional) Optional callback, returns an error object as first parameter and the result as second.

.. note:: The 2. ``address`` parameter can also be an address or index from the :ref:`chain3.mc.accounts.wallet <eth_accounts_wallet>`. It will then sign locally using the private key of this account.
...
1. ``Object`` - The transaction data to sign :ref:`chain3.mc.sendTransaction() <mc-sendtransaction>` for more.
...
``Promise<object>`` - The RLP encoded transaction. The ``raw`` property can be used to send the transaction using :ref:`chain3.mc.sendSignedTransaction <mc-sendsignedtransaction>`.
...
1. ``Object`` - A transaction object see :ref:`chain3.mc.sendTransaction <mc-sendtransaction-return>`, with the difference that for calls the ``from`` property is optional as well.
2. ``Number|String`` - (optional) If you pass this parameter it will not use the default block set with :ref:`chain3.mc.defaultBlock <mc-defaultblock>`.
3. ``Function`` - (optional) Optional callback, returns an error object as first parameter and the result as second.
...
It doesn't work if you're connected to a node with a default Chain3.js provider. (WebsocketProvider, HttpProvidder and IpcProvider)
...
.. code-block:: javascript
    ...
    chain3.mc.getChainId().then(console.log);
    > 61
...
.. code-block:: javascript
    ...
    chain3.mc.getNodeInfo().then(console.log);
...
2. ``Array`` 32 Bytes: Array of storage-keys which should be proofed and included. See :ref:`chain3.mc.getStorageAt <mc-getStorageAt>`.
...
``Promise<Object>`` - A account object.
    ...
    ``balance`` - The balance of the account. See :ref:`chain3.mc.getBalance <mc-getBalance>`.
